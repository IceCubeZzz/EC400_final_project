...
def load_dataset(dataset_path=DATASET_PATH, transform=dense_transforms.ToTensor()):
    return SuperTuxDataset(dataset_path, transform=transform)
    ...
    for track in args.track:
        ...
        while n < args.steps_per_track:
            steps, how_far = pytux.rollout(
                track, noisy_control, planner=planner, max_frames=1000, verbose=args.verbose, data_callback=collect)
            print(steps, how_far)
