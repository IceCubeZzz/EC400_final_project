...
from kart import Kart as DdqnKart
...
class PyTux:
    ...
    def rollout(self, track, controller, logger, planner=None, max_frames=1000, verbose=False, data_callback=None):
        """
        ...
        # initialize Kart
        dkart = DdqnKart(INPUT_DIM, FEAT_ACTIONS, "saved_checkpoints")
        ...
        for t in range(max_frames):
            ...
            if np.isclose(kart.overall_distance / track.length, 1.0, atol=2e-3):
                # remember how its done
                dkart.cache(image, next_image, action_num, reward, 1)
                if verbose:
            ...
            # apply ddqn to predict action
            image = np.array(self.k.render_data[0].image, dtype=np.single)
            image = np.moveaxis(image, [0, 1], [1, 2])
            action_num = dkart.act(image)
            action, reward = controller(aim_point_image, current_vel, last_rescue, track, action_num)
            ...
            # remember
            next_image = np.array(self.k.render_data[0].image, dtype=np.single)
            next_image = np.moveaxis(next_image, [0, 1], [1, 2])
            dkart.cache(image, next_image, action_num, reward, 0)
            ...
            t += 1
        return t, kart.overall_distance / track.length, dkart.exploration_rate, dkart.curr_step
...
if __name__ == '__main__':
    ...
    def noisy_control(aim_pt, vel, last_rescue, track):
        return control(aim_pt + np.random.randn(*aim_pt.shape) * aim_noise,
                       vel + np.random.randn() * vel_noise, last_rescue, track)

    def ddqn_control(aim_pt, vel, last_rescue, track, action_num):
        return control(aim_pt, vel, last_rescue, track, action_num)
    ...
    for track in args.track:
        ...
        # initialize logger 
        save_dir = Path("saved_checkpoints") /  datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S")
        ...
        while n < args.steps_per_track:
            ...
            if n % 20 == 0:
                logger.record(episode=args.steps_per_track, epsilon=exploration_rate, step=curr_step)
